...
# --- OCRパターン外部辞書（config/ocr_patterns.json） ---------------------------------------


def test_config_ocr_patterns_file_exists():
    config_path = REPO_ROOT / "config" / "ocr_patterns.json"
...
def test_docs_13_explains_external_pattern_dictionary_growth():
    """docs/13にconfig/ocr_patterns.jsonの育て方が書かれていることを確認する。"""
    text = _read(REPO_ROOT / "docs" / "13_ocr_quality_check_workflow.md")
    assert "config/ocr_patterns.json" in text
    assert "high_confidence_replacements" in text
    assert "delete_candidates" in text
    assert "inferred_candidates" in text
    assert "source_check_required" in text
    assert "allowed_words" in text


def test_docs_13_warns_against_over_correction():
    text = _read(REPO_ROOT / "docs" / "13_ocr_quality_check_workflow.md")
    assert "自動修正しすぎない" in text
    assert "固有名詞" in text
