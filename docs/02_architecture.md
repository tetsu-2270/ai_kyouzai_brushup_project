...
src/
  cli.py                     # CLI入口。import-source / build-all / regenerate / lesson-pages / review-report /
                              # generate / canva / docx / pdf / scenario / canva-sync / wp-publish の12サブコマンド
...
output/                       # (Git管理対象外) 実行結果の生成物置き場。すべて再生成可能な派生物・中間ファイル
  ...
  exports/                      # PDF/PPTX/DOCX/Markdown（Phase 9）
  compat/                       # Phase 8互換output。editable//canva/との同名重複を避けるため分離（Phase 9.1）
    lesson_pages.json            # editable/lesson_pages.jsonの後方互換コピー（`--no-compat-output`で無効化可）
