...
- 将来のLLM連携（プロジェクト方針上、優先はローカルLLM）を前提に、プロンプト生成まで行う

本格的なAI本文生成を行う場合は、別フェーズで LLM連携レイヤーを設計します（14節参照）。
...
**プロジェクト方針（README.md「プロジェクト方針：外部API非依存・ローカルLLM移行前提」・[`docs/07_api_integration_design.md`](07_api_integration_design.md)参照）のとおり、将来的な自動化の中心はローカルLLMであり、外部API連携は必要になった場合の選択肢にとどめる。**

### 14.1 LLM連携レイヤー

- ローカルLLM（優先して検証する対象）
- OpenAI API（必要になった場合の選択肢）
- Claude API（必要になった場合の選択肢）
- Gemini API（必要になった場合の選択肢）
