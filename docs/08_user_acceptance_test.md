...
### 6.7 Claude Codeレビュー候補の反映（`apply-ocr-review`）の確認項目

`claude_review/candidates.json`が生成できたら、`apply-ocr-review`で`editable/lesson_pages.json`へ反映できます。詳細は[`docs/16_apply_ocr_review_workflow.md`](16_apply_ocr_review_workflow.md)を参照してください。

- [ ] `apply-ocr-review --output-dir <output-dir> --dry-run`が実行でき、`OCR_REVIEW_APPLY_DRY_RUN: passed`と変更予定ページ一覧が表示されることを確認する
- [ ] dry-run実行後も`editable/lesson_pages.json`が変更されておらず、バックアップも作成されていないことを確認する
- [ ] `apply-ocr-review --output-dir <output-dir> --apply`実行後、`editable/backups/`にバックアップが1件作成され、`editable/lesson_pages.json`のtitle/body/summary/image_text/canva_prompt/video_sceneに`proposed_text`の内容が反映されていることを確認する
- [ ] `layout_instruction`・`source_image`・`source_assets`・`source_page_no`・`role`・`page_no`・`metadata`が変更されていないことを確認する
- [ ] 同じ候補で`--dry-run`を再実行すると「変更予定ページ: なし」となり、`--apply`を再実行してもバックアップが増えないこと（冪等性）を確認する
- [ ] `unresolved`な候補ページが含まれる場合、`apply-ocr-review`が非ゼロ終了コードで失敗し、`editable/lesson_pages.json`が変更されないことを確認する
- [ ] 反映後に`regenerate --output-format all`を実行し、`canva/canva_design.md`等の完成outputに反映内容が反映されていることを確認する

### 6.8a 教材本文ブラッシュアップ（`prepare-content-brushup` / `apply-content-brushup`）の確認項目

OCR確定原文を証拠として保持したまま、教材本文の分かりやすさを改善できます。詳細は[`docs/18_content_brushup_workflow.md`](18_content_brushup_workflow.md)を参照してください。

- [ ] `prepare-content-brushup --output-dir <output-dir>`が実行でき、`CONTENT_BRUSHUP_PREPARE: passed`とコピー可能な1文が表示されることを確認する
- [ ] `content_brushup/VERIFIED_OCR_SNAPSHOT.json`が生成され、`source_sha256`が記録されていることを確認する
- [ ] 再実行してもスナップショットが最新であれば据え置かれ、既存の作業中候補（あれば）が破棄されないことを確認する
- [ ] `lesson_pages.json`を書き換えた状態で再実行すると、`--force`無しではエラーで拒否され、`--force`を指定すると作り直されることを確認する
- [ ] 指示書をClaude CodeまたはCodexへ渡し、追加の質問なしにページ別候補JSON（`content_brushup/pages/page_NNN.json`）が1ページずつ保存されることを確認する
- [ ] 候補JSONの`original`がスナップショットと完全一致し、本文が複製されるだけでなく実際に読みやすさが改善された`proposed`が作成されていることを確認する
- [ ] `apply-content-brushup --output-dir <output-dir> --dry-run`が実行でき、`CONTENT_BRUSHUP_APPLY_DRY_RUN: passed`と変更予定ページ・変更なしページの一覧が表示されることを確認する
- [ ] dry-run実行後も`editable/lesson_pages.json`・`VERIFIED_OCR_SNAPSHOT.json`が変更されておらず、バックアップも作成されていないことを確認する
- [ ] `review.html`を開き、原文と改善案が文字単位の差分付きで並んで表示され、「OCR確定原文は変更されていません」という注意書きがあることを確認する
- [ ] `risk_level: high`または`requires_human_review: true`の候補が含まれる場合、`apply-content-brushup`が非ゼロ終了コードで失敗し、`editable/lesson_pages.json`が変更されないことを確認する
- [ ] `apply-content-brushup --output-dir <output-dir> --apply`実行後、`editable/backups/`にバックアップが1件作成され、`title`/`body`/`summary`と派生field（`image_text`/`canva_prompt`/`video_scene`）が改善案の内容に整合していることを確認する
- [ ] `page_no`/`source_page_no`/`source_image`/`layout_instruction`/`notes`/`metadata`が変更されていないことを確認する
- [ ] 同じ候補で`--dry-run`/`--apply`を再実行しても正しく「変更なし」となり、バックアップが増えないこと（冪等性）を確認する
- [ ] `--apply`成功後、次に`prepare-image-brushup`を実行するよう案内が表示されることを確認する
- [ ] 本文更新後、古い`design_manifest.json`のまま`render-brushup`を実行すると拒否され、`prepare-image-brushup`の再実行を促されることを確認する

### 6.8 教材画像ブラッシュアップ生成（`prepare-image-brushup` / `render-brushup`）の確認項目

確定済みの本文（`editable/lesson_pages.json`）を使って、実際に見た目を再設計した教材画像を生成できます。詳細は[`docs/17_image_brushup_workflow.md`](17_image_brushup_workflow.md)を参照してください。

- [ ] `prepare-image-brushup --output-dir <output-dir>`が実行でき、`IMAGE_BRUSHUP_PREPARE: passed`とコピー可能な1文が表示されることを確認する
- [ ] `brushup_design/AI_IMAGE_BRUSHUP.md`・`README.md`が生成され、本文（title/body/summary）が指示書へ埋め込まれていないことを確認する
- [ ] 指示書をClaude CodeまたはCodexへ渡し、追加の質問なしに元画像を視覚確認しながらページ別デザインJSON（`brushup_design/pages/page_NNN.json`）が1ページずつ保存されることを確認する
- [ ] デザインJSONが本文を複製せず、`source_field`で`lesson_pages.json`を参照する形式になっていることを確認する
- [ ] 全ページ完了後、`design_manifest.json`が生成されることを確認する
- [ ] `render-brushup --output-dir <output-dir>`が実行でき、`IMAGE_BRUSHUP_RENDER: passed`と生成ページ数が表示されることを確認する
- [ ] `rendered_brushup/page_NNN.png`が生成され、既存の`rendered/`には一切書き込まれていないことを確認する
- [ ] 生成画像が元画像の単純コピーではない（ファイルハッシュ・見た目が異なる）ことを確認する
- [ ] `comparison.html`を開き、元画像とブラッシュアップ画像を並べて、本文の省略・欠落・要約が無いことを目視で確認する
- [ ] ページ内容に応じてレイアウト（テンプレート）が変わり、かつページ間で配色・フォント等の統一感があることを確認する
- [ ] `render_report.md`で失敗ページが無いこと（オーバーフローで打ち切られたページが無いこと）を確認する
- [ ] `editable/lesson_pages.json`・元画像・`assets/`がいずれも変更されていないことを確認する

### 6.8b 統一スライドマスター・Codex向け最終画像生成パッケージ（`prepare-final-image-package`）の確認項目

全ページ共通のスライドマスターと、Codex（Phase 10.15・次工程）向けの最終画像生成パッケージを生成できます。詳細は[`docs/19_final_image_package_workflow.md`](19_final_image_package_workflow.md)を参照してください。**この工程では完成画像は生成されません。**

- [ ] `brushup_design`（Phase 10.12）が未整備・古い場合、`prepare-final-image-package`が非ゼロ終了コードで失敗し、`prepare-image-brushup`/`render-brushup`の実行を促されることを確認する
- [ ] `prepare-final-image-package --output-dir <output-dir>`が実行でき、`FINAL_IMAGE_PACKAGE_PREPARE: passed`・共通キャンバスサイズ・共通本文カード座標・Codex向けのコピー可能な1文が表示されることを確認する
- [ ] `final_image_package/MASTER_LAYOUT.json`が生成され、全ページ分のページ別仕様（`pages/page_NNN.json`）の`master_layout`・`source_lesson_pages_sha256`と一致することを確認する
- [ ] 全ページの`content_card`（x/y/width/height/corner_radius）・`notice_region`・`page_number_region`が完全に同一であることを確認する（`MASTER_LAYOUT.json`1件のみから供給されるため構造的に保証される）
- [ ] 内容量が少ないページと多いページで、本文カードの外形（大きさ・位置）が変わっていないことを、`final_image_package/preview/comparison.html`で目視確認する
- [ ] `final_image_package/preview/master_guides.png`を開き、キャンバス・タイトル領域・本文カード・注記領域・ページ番号領域が座標付きで可視化されていることを確認する
- [ ] `final_image_package/text/page_NNN.json`の本文が、OCR確定原文ではなく（Phase 10.13で本文ブラッシュアップ済みの場合は）改善後の本文になっていることを確認する
- [ ] `final_image_package/CODEX_FINAL_IMAGE_GENERATION.md`に、「一切の文字を生成しない」「本文カード等の固定領域に装飾を置かない」ことが明記されていることを確認する
- [ ] `final_image_package/emphasis`（各ページ仕様内）の`match`が、実在しない文言の場合にパッケージ生成が失敗することを確認する
- [ ] `output/<output-dir>/rendered_final/`がこの工程では生成されていないことを確認する
- [ ] `rendered_brushup_preview/`が新規生成され、既存の`rendered/`・`rendered_brushup/`・`editable/lesson_pages.json`・元画像がいずれも変更されていないことを確認する
