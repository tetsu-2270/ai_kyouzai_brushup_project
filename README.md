...
- **動作確認: Python 3.9.6でもpytest全件(527件)が通ることを確認済み**（`src/`内の`str | Path`等のPEP604構文は、全ファイルに`from __future__ import annotations`を付与済みのため、型注釈の評価が遅延され3.9でも動作する）
...
`llm-handoff`でLLMへ改善依頼をする前に、OCR誤認識・文字化け・不自然な表記の可能性がある箇所をシステム側で検出し、修正候補・重要度をまとめたレポート（`ocr_check_report.md`）と、将来の自動反映に使える補正候補JSON（`ocr_correction_candidates.json`）を出力します（**自動修正・自動反映は行いません**）。候補は「high confidence correction（修正後がほぼ一意）」「deletion candidate（削除候補）」「inferred correction candidate（推定修正候補）」「source check required（元画像確認必須）」の4分類で提示され、単に「元画像確認」で終わらせず判断しやすくしています。検出に使う辞書は`config/ocr_patterns.json`で管理でき、コードを変更せずに実データから辞書を育てられます（無くても組み込みデフォルトで動作）。詳細は[`docs/13_ocr_quality_check_workflow.md`](docs/13_ocr_quality_check_workflow.md)を参照してください。
