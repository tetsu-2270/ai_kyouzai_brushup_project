...
- **動作確認: Python 3.9.6でもpytest全件(648件)が通ることを確認済み**（`src/`内の`str | Path`等のPEP604構文は、全ファイルに`from __future__ import annotations`を付与済みのため、型注釈の評価が遅延され3.9でも動作する）
...
- **Apple Vision OCR（`--ocr-engine tesseract+vision`）はmacOS専用・完全に任意の機能です。** Tesseractは既定のまま維持されており、Apple Visionを使わない限り（`--ocr-engine`を指定しない限り）動作要件・挙動に変化はありません。使う場合はmacOS + Xcode Command Line Tools（Swiftツールチェーン）が必要で、`bash scripts/build_apple_vision_ocr.sh`でローカルビルドしたヘルパー（`tools/apple_vision_ocr/`）を使用します。処理はすべてローカルのVisionフレームワーク内で完結し、画像やOCR結果を外部へ送信しません。詳細は「Apple Vision OCRとの比較」節を参照してください。
...
**元資料を差し替えて最初から作り直す場合は`--clean-output`を使ってください。** 前回より少ないページ数（例: 52ページ→13ページ）で再生成すると、`--clean-output`を指定しない限り前回分の古いページ画像・成果物が`output_dir`配下に残り続けます（上書きだけでは古いファイル名の分だけ消えずに混在する）。

```bash
python3 -m src.cli build-all --input input/source --mode proofread --output-dir output --output-format all --clean-output
```

`--clean-output`は、取り込み・生成の**前**に`output_dir`配下の**既知の生成物のみ**（`assets/`・`editable/`・`compat/`・`scenario/`・`rendered/`・`exports/`・`canva/`・`imported_pages.json`・`review_report.md`・`ocr_check_report.md`・`ocr_correction_candidates.json`・`llm_handoff.md`）を削除してから再生成します。`output_dir`配下にある、システムが生成したものではない手動ファイル・ディレクトリには一切触れません。既定では無効（従来どおり上書きのみ）です。安全のため、`output_dir`がプロジェクトディレクトリ配下または`/tmp`配下であること、`input/`・`.git/`・`src/`・`tests/`・`docs/`と重ならないことを検証し、条件を満たさない場合は削除を行わずエラーにします（実装は`src/output_clean.py`）。

**あわせて、Phase 8時点の旧仕様output（`output_dir`直下の`lesson_pages.json`・`canva_design.md`・`brushup.md`・`brushup.docx`・`brushup.pdf`）も削除対象です。** これらは現行の`build-all`はもう生成しません（Phase 9で`output/compat/`配下にまとめられました）が、旧バージョンでの実行結果や個別コマンドの手動実行結果が残っていると`output/compat/`配下の現行版と紛らわしく誤参照の原因になるため、既知の生成物として削除対象に含めています。一方、`apply-ocr-corrections`/`approve-ocr-candidates`の生成物（`ocr_apply_report.md`等）、`lesson-pages --plan-output`で出力する`restructure_plan.json`（任意のファイル名を指定できるオプション出力）、固定の既定パスを持たないカスタム名の出力ファイルは、手動ファイルと区別できない・OCR承認ワークフローの範囲外であるため削除対象に含めていません。

### Apple Vision OCRとの比較（`--ocr-engine`。macOS専用・任意）

`build-all`の既定（`--ocr-engine tesseract`。省略時と同じ）は従来通りTesseractのみを使用します。`--ocr-engine tesseract+vision`を指定すると、画像inputの場合に限り、macOS標準のVisionフレームワーク（`VNRecognizeTextRequest`）による独立した第二のOCR結果も取得し、Tesseract結果と比較します。
...
### OCR品質チェック・補正候補データを生成（`ocr-check`）
...
`llm-handoff`でLLMへ改善依頼をする前に、OCR誤認識・文字化け・不自然な表記の可能性がある箇所をシステム側で検出し、修正候補・重要度をまとめたレポート（`ocr_check_report.md`）と、将来の自動反映に使える補正候補JSON（`ocr_correction_candidates.json`）を出力します（**自動修正・自動反映は行いません**）。候補は「high confidence correction（修正後がほぼ一意）」「deletion candidate（削除候補）」「inferred correction candidate（推定修正候補）」「source check required（元画像確認必須）」の4分類で提示され、単に「元画像確認」で終わらせず判断しやすくしています。同じOCR崩れから重複して出た候補は自動で整理され（重複抑制）、レポートには抑制前後の件数が表示されます。検出に使う辞書は`config/ocr_patterns.json`で管理でき、コードを変更せずに実データから辞書を育てられます（無くても組み込みデフォルトで動作）。詳細は[`docs/13_ocr_quality_check_workflow.md`](docs/13_ocr_quality_check_workflow.md)を参照してください。

### 承認済みOCR補正候補を反映（`apply-ocr-corrections`）

```bash
python3 -m src.cli apply-ocr-corrections --input output/editable/lesson_pages.json --candidates output/ocr_correction_candidates.json --output output/editable/lesson_pages.ocr_fixed.json --report output/ocr_apply_report.md
```

`ocr_correction_candidates.json`のうち、人間が`status: approved`に変更した候補だけを`lesson_pages.json`へ反映します（**`--input`の元ファイルは上書きしません**。`proposed`/`rejected`/`needs_image_check`/`needs_source_check`/`needs_human_review`の候補やOCR候補の自動承認は行いません。削除候補（`action: delete`）は`approved`にしても今回は反映されません）。詳細は[`docs/14_apply_ocr_corrections_workflow.md`](docs/14_apply_ocr_corrections_workflow.md)を参照してください。

### 高重要度OCR候補を一括approved化（`approve-ocr-candidates`）

```bash
python3 -m src.cli approve-ocr-candidates --input output/ocr_correction_candidates.json --output output/ocr_correction_candidates.approved.json --report output/ocr_approval_report.md --severity high --action replace --confidence high
```
...
### LLM改善案を構造化候補に変換（`apply-llm-suggestions`）

```bash
python3 -m src.cli apply-llm-suggestions --lesson-pages output/editable/lesson_pages.json --suggestions output/llm_response.md --candidates-output output/llm_suggestion_candidates.json --report output/llm_suggestion_report.md
```

ChatGPT/Claude等から返ってきた改善案Markdownを読み込み、ページ・項目ごとの改善候補として構造化します（**`lesson_pages.json`への自動反映・LLM改善案の自動採用は行いません**。すべての候補は`status: proposed`で生成され、人間が採用判断します）。詳細は[`docs/15_llm_suggestion_candidates_workflow.md`](docs/15_llm_suggestion_candidates_workflow.md)を参照してください。

### LLM回答の採用判断シートを生成（`edit-plan-template`）

```bash
python3 -m src.cli edit-plan-template --input output/editable/lesson_pages.json --output output/edit_plan_template.md
```

ChatGPT/Claude等から返ってきた改善案を、そのまま`editable/lesson_pages.json`へ反映する前に、ページごとの「採用する／採用しない／一部採用」を整理するための採用判断シートを生成します。使い方の詳細は[`docs/12_llm_review_apply_workflow.md`](docs/12_llm_review_apply_workflow.md)を参照してください。
...
### 検証結果を永続保存する（`run_verification.sh`）

`pytest`単体・`run_sample.sh`単体の実行結果は画面に表示されるだけで、通常は保存されません。Claude Code等が実装後の検証結果をCodex等の別セッションから後で確認できるようにしたい場合は、以下の正式な検証入口を使ってください。

```bash
bash scripts/run_verification.sh --purpose "実装した機能の検証"
```

`python3 -m pytest -q --junitxml=...` → `bash scripts/run_sample.sh` の順に実行し、結果を`logs/evidence/<run_id>/`（`manifest.json`/`summary.md`/コマンドログ/JUnit XML）へ保存します。過去の実行結果は上書きしません。最新の完了済み結果は`logs/evidence/latest.json`が指します。実行結果本体はGit管理対象外です（`logs/evidence/.gitkeep`のみ対象）。詳細は[`docs/04_output_spec.md`](docs/04_output_spec.md)「検証エビデンス」を参照してください。
...
`output/`配下のディレクトリ構成（`editable/`/`rendered/`/`canva/`/`exports/`/`compat/`）は、Phase 9.2までに確定したプロジェクト共通設計ルールです。詳細は[`docs/04_output_spec.md`](docs/04_output_spec.md)「プロジェクト標準output構成」、要約は[`PROJECT_RULES.md`](PROJECT_RULES.md)「4. output構成」を参照してください。
...
src/
  ...
  ocr_engine.py                # 教材画像向けOCR品質改善エンジン。複数前処理・複数PSM・信頼度/座標付き
                              # 結果からの品質スコアによる最良候補選択・低品質時のみの再試行・ノイズ除去・
                              # 辞書補正を行う（import_source.pyの_try_ocr()から呼ばれる）
  output_clean.py             # build-all --clean-output用。output-dir配下の既知の生成物
                              # （assets/・editable/・compat/等）だけを安全に削除する（詳細は後述「--clean-output」）
  ...
  verification_evidence.py    # pytest/run_sample.sh等の検証結果をlogs/evidence/<run_id>/へ永続保存する
                              # ライブラリ（manifest.json/summary.md/コマンドログ/JUnit XML。過去結果は上書きしない）
  verification_runner.py      # scripts/run_verification.shから呼ばれる検証入口の本体。
                              # pytest --junitxml → run_sample.sh の順に実行しエビデンスへ記録する
  ocr_check.py                 # OCR品質チェック・補正候補データ生成（ocr_check_report.md /
                              # ocr_correction_candidates.json）。自動修正・自動反映は行わない（詳細はdocs/13参照）
  ocr_patterns.py               # OCR崩れ検出辞書の管理。config/ocr_patterns.json（外部辞書、無くても
                              # 動く）と組み込みデフォルトを安全にマージする（詳細はdocs/13参照）
  ocr_apply.py                  # ocr_correction_candidates.jsonのstatus: approved候補だけを
                              # lesson_pages.jsonへ反映。元ファイル上書き・自動承認はしない（詳細はdocs/14参照）
  ocr_approval.py               # ocr_correction_candidates.jsonの条件一致候補（既定: 高重要度・高確信度の
                              # replace候補）だけを一括status: approvedに変更。lesson_pages.jsonへの
                              # 反映は行わない（詳細はdocs/14参照）
  llm_handoff.py               # editable/lesson_pages.jsonから、ChatGPT/Claude等へ手作業で貼り付けるための
                              # Markdownを生成（LLM出力の自動取り込みは行わない。詳細はdocs/11参照）
  llm_suggestions.py            # LLM改善案Markdownをページ別の改善候補JSON/レポートに構造化。
                              # lesson_pages.jsonへの自動反映は行わない（詳細はdocs/15参照）
  edit_plan.py                 # LLM改善案の採用判断シート（edit_plan_template.md）を生成（詳細はdocs/12参照）
...
scripts/
  ...
  run_verification.sh         # 正式な検証入口。pytest→run_sample.shを実行し結果をlogs/evidence/へ保存
...
このプロジェクトの開発ルールは、以下の3階層に分かれています。同じルールを複数ファイルへ重複記載せず、それぞれ次の正本を参照してください。

1. **全開発共通ルール**（ChatGPT/CodexとClaude Codeで共通）: `~/ai-development-rules/DEVELOPMENT_RULES.md`（ホームディレクトリ配下・個人環境の設定）
2. **エージェント固有ルール**: Codexは`~/.codex/AGENTS.md`、Claude Codeは`~/.claude/CLAUDE.md`（いずれもホームディレクトリ配下）
3. **プロジェクト固有ルール**: [`PROJECT_RULES.md`](PROJECT_RULES.md)（本リポジトリ直下・Git管理対象）。エージェントごとの薄い入口は[`AGENTS.md`](AGENTS.md)（Codex用）・`CLAUDE.md`（Claude Code用）。

**共通ルール（1・2）が存在しない環境（別マシン・別ユーザーへclone した場合等）では、`PROJECT_RULES.md`と各エージェントのプロジェクト固有ファイル（`AGENTS.md`/`CLAUDE.md`/`CLAUDE_RULES.md`）だけで安全に作業できます。** 共通ルールが無いことだけを理由に実装を停止しないでください。安全上必要な最低限のルール（Git commit/push・秘密情報・破壊的操作の事前確認等）は、いずれもGit管理対象のプロジェクト固有ルールに含まれています。
...
詳細手順は [`docs/06_claude_code_workflow.md`](docs/06_claude_code_workflow.md)、Claude Code固有の実装手順は `CLAUDE_RULES.md`、このプロジェクト固有ルールは `PROJECT_RULES.md` を参照してください。ドキュメント全体の見取り図は [`docs/README.md`](docs/README.md) を参照してください。

## Codexで設計する場合

Codexがこのプロジェクトで設計・調査を行う場合は、[`AGENTS.md`](AGENTS.md)（薄い入口）→ `~/.codex/AGENTS.md`（存在する場合）→ `PROJECT_RULES.md` の順に読み込みます。Claude Code向け実装指示文の書き方も`AGENTS.md`から辿れます。
