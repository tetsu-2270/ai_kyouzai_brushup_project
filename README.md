...
**教材の作成者は、JSON・Markdown・TXTを手作業で作る必要はありません。** 元資料（画像・PDF・PPTX）を`input/source/`に置いて`build-all`コマンドを実行するだけで、教材ブラッシュアップ成果物一式が自動生成されます。
...
内部では「元資料からのテキスト・画像自動取り込み（`import-source`）→正データ生成（`lesson-pages`）→各種成果物生成」が自動的に実行され、`output/lesson_pages.json`・`output/brushup.md`・`output/canva_design.md`・`output/brushup.docx`・`output/brushup.pdf`・`output/scenario/`一式が生成されます。詳しい手順・確認観点は[`docs/08_user_acceptance_test.md`](docs/08_user_acceptance_test.md)を参照してください。
...
- **動作確認: Python 3.9.6でもpytest全件(197件)が通ることを確認済み**（`src/`内の`str | Path`等のPEP604構文は、全ファイルに`from __future__ import annotations`を付与済みのため、型注釈の評価が遅延され3.9でも動作する）
...
- **OCR（画像・PDFからのテキスト抽出）にはOS側にtesseract本体のインストールが必要です**（例: macOSなら`brew install tesseract tesseract-lang`）。`pytesseract`はtesseract本体を呼び出すPythonラッパーであり、`pip install`だけでは実際のOCRは動作しません。tesseract本体が無い環境でも`import-source`/`build-all`自体は失敗せず、テキストが空のまま元画像を保持して取り込みます（詳細は`docs/08_user_acceptance_test.md`「OCRについての注意」参照）。
...
作成者は以下ではなく、前述「クイックスタート（作成者向け）」の`build-all`を使ってください。以下は開発者・拡張作業向けに、個別コマンドの使い方を示します（いずれも`--input`に`output/lesson_pages.json`を指定する前提で記載する）。
...
`import-source` → `lesson-pages` → `generate`/`canva`/`docx`/`pdf`/`scenario`/`review-report`を内部で順に実行します。`--mode`は`proofread`/`restructure`、`--requirements`は`restructure`時のみ任意で指定できます。
...
src/
  cli.py                     # CLI入口（import-source / build-all / lesson-pages / review-report / generate / canva / docx / pdf / scenario / canva-sync / wp-publish サブコマンド）
  ...
  canva_renderer.py          # lesson_pages.jsonからcanva_design.md生成
  docx_renderer.py           # lesson_pages.jsonからbrushup.docx生成
  pdf_renderer.py            # lesson_pages.jsonからbrushup.pdf生成
...
output/                       # (Git管理対象外) 実行結果の生成物置き場。すべてシステムが生成する派生物・中間ファイル
  ...
  assets/                     # 元画像・元ページ画像・スライド埋め込み画像（Canva設計で参照する）
                              # lesson_pages.json/brushup.md/canva_design.md/brushup.docx/brushup.pdf/
                              # scenario//restructure_plan.json/review_report.md等もここに生成される
